...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    private func addObservers() {
        let keyValueObservation = session.observe(\.isRunning, options: .new) { _, change in
            ...
            let isSemanticSegmentationMatteEnabled: Bool
            if #available(iOS 13.0,*) {
                isSemanticSegmentationMatteEnabled = !self.photoOutput.enabledSemanticSegmentationMatteTypes.isEmpty
            } else {
                isSemanticSegmentationMatteEnabled = false
            }
...
@available(iOS 13.0, *)
extension CameraViewController: ItemSelectionViewControllerDelegate {

    var semanticSegmentationTypeItemSelectionIdentifier: String { return "SemanticSegmentationTypes" }
...
/*
 *
 * MARK: copied back from Xcode 11.0b4
 *
 */
...
extension AVCapturePhotoOutput {
    ...
    var enabledSemanticSegmentationMatteTypes: [AVSemanticSegmentationMatte.MatteType] {    get { return [] }           set { } }
    var maxPhotoQualityPrioritization: AVCapturePhotoOutput.QualityPrioritization {         get { return .balanced }    set { } }
    var embedsSemanticSegmentationMattesInPhoto: Bool {                                     return false }
    var photoQualityPrioritization: AVCapturePhotoOutput.QualityPrioritization {            return .balanced }
    var availableSemanticSegmentationMatteTypes: [AVSemanticSegmentationMatte.MatteType] {  return [] }
}

@available(iOS 13.0, *)
extension AVCaptureResolvedPhotoSettings {
    var photoProcessingTimeRange: CMTimeRange { return CMTimeRange(start: CMTime(seconds: 0, preferredTimescale: 1),
                                                                   duration: CMTime(seconds: 4, preferredTimescale: 1)) }
}

@available(iOS 13.0, *)
extension AVCapturePhotoSettings {
    var enabledSemanticSegmentationMatteTypes: [AVSemanticSegmentationMatte.MatteType] {    get { return [] }           set { } }
    var photoQualityPrioritization: AVCapturePhotoOutput.QualityPrioritization {            get { return .balanced }    set { } }
}

@available(iOS 13.0, *)
class AVSemanticSegmentationMatte : NSObject {
    public struct MatteType : Hashable, Equatable, RawRepresentable {
        public var rawValue: String
        public init(rawValue: String) { self.rawValue = rawValue }
        static let skin: MatteType = MatteType(rawValue: "skin")
        static let hair: MatteType = MatteType(rawValue: "hair")
        static let teeth: MatteType = MatteType(rawValue: "teeth")
    }

    var matteType: AVSemanticSegmentationMatte.MatteType = .skin
}

@available(iOS 13.0, *)
extension UIWindow {
    weak var windowScene: UIWindowScene? {                                              return UIWindowScene() }
}

@available(iOS 13.0, *)
class UIWindowScene  {
    var interfaceOrientation: UIInterfaceOrientation {                                  return UIApplication.shared.statusBarOrientation }
}
